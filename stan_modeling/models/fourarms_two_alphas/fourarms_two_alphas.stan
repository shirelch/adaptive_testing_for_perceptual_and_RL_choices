...
parameters {
  ...
  //individuals level
  vector[Nsubjects] alpha_pos_random_effect;
  vector[Nsubjects] alpha_neg_random_effect;
...
transformed parameters {
  ...
  for (subject in 1:Nsubjects) {
    ...
    alpha_pos[subject]   = inv_logit(population_locations[1]  + population_scales[1] * alpha_pos_random_effect[subject]);
    alpha_neg[subject]   = inv_logit(population_locations[2]  + population_scales[2] * alpha_neg_random_effect[subject]);
    beta[subject]    =          (population_locations[3]  + population_scales[3] * beta_random_effect [subject]);
        ...
        Qval[choice[subject,trial]] = Qval[choice[subject,trial]]+alpha_pos[subject]*PE;
        }
        else{
          Qval[choice[subject,trial]] = Qval[choice[subject,trial]]+alpha_neg[subject]*PE;
        }
...
model {
  ...
  // indvidual level  
  alpha_pos_random_effect ~ std_normal();
  alpha_neg_random_effect ~ std_normal();
